...
async function generatePDF(files, fontName, fontSize, outputPath, newPagePerFile = true, showLineNumbers = true) {
    ...
    // Load Noto font for UTF-8 support
    const fontPath = path.join(__dirname, '..', 'assets', 'NotoSansMono-VariableFont_wdth,wght.ttf');
    const fontBytes = fs.readFileSync(fontPath);
    const font = await pdfDoc.embedFont(fontBytes);
    const boldFont = await pdfDoc.embedFont(fontBytes); // same font for bold fallback
